...
Функция ПолучитьЗаголовки()
	...
	Заголовки.Вставить("X-Api-Key",Whatsgate_Сервер.ПолучитьТокенПользователя());
...
Функция ПометитьКакПрочитанное(Данные = Неопределено) Экспорт
	...
	//
	ДанныеТела = Новый Структура;
		ДанныеТела.Вставить("WhatsappID",Whatsgate_Сервер.ПолучитьИдентификаторСессии());
		//
		Получатель = Новый Структура;
...
КонецФункции
//
Функция ПолучитьПрикрепленныйМедиафайл(КлючМедиа = Неопределено) Экспорт  
...
Функция СоздатьНовуюСессию(НазваниеСессии = Неопределено) Экспорт 
...
КонецФункции
//
Функция УдалитьВсеСессии() Экспорт 
	//
	HTTPСоединение = ПолучитьСоединение();
	//
	ДанныеТела = Новый Структура;
...
КонецФункции
//
Функция ИзменитьИмяСессии(НазваниеСессии = Неопределено) Экспорт 
...
Функция УстановитьWebhook(Обработчик = Неопределено) Экспорт
	...
	ДанныеТела = Новый Структура;
	ДанныеТела.Вставить("WhatsappID",Whatsgate_Сервер.ПолучитьИдентификаторСессии());
	ДанныеТела.Вставить("callback",Обработчик);
	//
	Параметры = Новый Структура;
	//
	HTTPЗапрос = Новый HTTPЗапрос(ВставитьПараметры("/api/v1/set-hook",Параметры),ПолучитьЗаголовки());
		HTTPЗапрос.УстановитьИмяФайлаТела(ПолучитьJSONФайл(ДанныеТела));
		Ответ = HTTPСоединение.ВызватьHTTPМетод("POST",HTTPЗапрос);
		//
	Если ОбработатьОтвет(Ответ) Тогда
...
Функция ПолучитьСтруктуруДанных(НомерСтруктуры = 0) Экспорт
	...
	Если НомерСтруктуры = 0 Тогда // Объект отправки сообщения
		Шаблон.Вставить("ИдентификаторОтправителя",Whatsgate_Сервер.ПолучитьИдентификаторСессии());
